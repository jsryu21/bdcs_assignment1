...
public final class AlsDriver {
  ...
  final class StartHandler implements EventHandler<StartTime> {
    ...
    public void onNext(final StartTime startTime) {
      final int numEvals = AlsDriver.this.numberOfReceivers + 1;
      LOG.log(Level.INFO, "Requesting {0} evaluators", numEvals);
      AlsDriver.this.requestor.submit(EvaluatorRequest.newBuilder()
          .setNumber(numEvals)
          .setMemory(2048)
          .build());
    }
      ...
      try {
        final Configuration contextConfiguration = ContextConfiguration.CONF
            .set(ContextConfiguration.IDENTIFIER, "BroadcastContext-" +
                AlsDriver.this.numberOfAllocatedEvaluators.getAndDecrement())
            .build();
